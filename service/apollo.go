...
func CreateBuild(namespace string, project *model.Project) {
  ...
  for _, tool := range buildTools {
    ...
    go func() {
      ...
      build(_tool, project, namespace)
    }()
  }
}

func build(build BuildTool, project *model.Project, namespace string) {
  buildId, err := build.Build(project)
  if buildId == 0 {
    // 项目不存在
    if err != nil {
  ...
  for {
    ...
    finished, err := build.IsFinish(buildId)
    if err != nil {
      log.Println("err: ", err)
      break
    }
    buildInfo.State = build.JobStatus()
